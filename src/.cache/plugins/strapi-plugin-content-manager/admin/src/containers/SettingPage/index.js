...
class SettingPage extends React.PureComponent {
    ...
    getAttrData = attrName =>
        get(
            this.getEditPageDisplaySettings(),
            ['availableFields', attrName],
            {}
        );
    ...
    getDropDownItems = () => {
        ...
        return Object.keys(attributes)
            ...
            .map(attr => {
                const searchable =
                    attributes[attr].type !== 'json' &&
                    attributes[attr].type !== 'array';
    ...
    getEditPageDisplayedFields = () =>
        get(this.getEditPageDisplaySettings(), ['fields'], []);

    getEditPageDisplayedRelations = () =>
        get(this.getEditPageDisplaySettings(), ['relations'], []);
    ...
    getEditPagePossibleEntryTitleFields = () => {
        ...
        const stringAndNumberFields = Object.keys(availableFields).filter(
            field => {
    ...
    getRelations = () => {
        ...
        return Object.keys(relations).filter(relation => {
            const isUploadRelation =
                get(relations, [relation, 'plugin'], '') === 'upload';
            const isMorphSide =
                get(relations, [relation, 'nature'], '')
                    .toLowerCase()
                    .includes('morph') &&
                get(relations, [relation, relation]) !== undefined;
    ...
    handleChange = e => {
        ...
        const isDisablingDefaultSort =
            attrName === defaultSort && e.target.value === false;
    ...
    manageRemove = (index, keys, itemName, data, isRelation = true) => {
        const isRemovingSelectedItem = isRelation
            ? itemName === data.alias
            : itemName === data.name;
        ...
        if (isRemovingSelectedItem) {
            ...
            const selectNextFunc = isRelation
            ...
            if (selectNextItemCond) {
                ...
                if (!isRelation) {
                    const nextItem = get(
                        this.getEditPageDisplayedFields(),
                        nextIndex
                    );
    ...
    findIndexFieldToEdit = () => {
        ...
        const index = this.getEditPageDisplayedFields().indexOf(
            fieldToEdit.name
        );
    ...
    toggleDropDownFields = () => {
        if (this.getDropDownFieldItems().length > 0) {
            this.setState(prevState => ({
                isOpenField: !prevState.isOpenField
            }));
        }
    };
    ...
    renderForm = () => {
        ...
        if (isEmpty(relationToEdit)) {
            return forms.editView.fieldForm.map(
                this.renderFormEditSettingsField
            );
        }
    ...
    renderFormListAttrSettings = (input, i) => {
        ...
        const inputName = `${this.getPath()}.listDisplay.${indexListItemToEdit}.${
            input.name
        }`;
                ...
                <div className="row">
                                ...
                                <div className={cn('col-md-6')}>
                                    <div
                                        className={cn(
                                            ...
                                            this.shouldDisplayCursorNotAllowed(
                                                'fields'
                                    ...
                                    >
                                        <ButtonDropdown
                                            isOpen={isOpenField}
                                            toggle={this.toggleDropDownFields}
                                        >
                                            <DropdownToggle>
                    ...
                    {this.hasRelations() && (
                        ...
                        >
                            ...
                            />
                            <div
                                className={cn(styles.sort_wrapper, 'col-md-12')}
                            >
                                <div className="row">
                                    {/* DRAGGABLE BLOCK */}
                                    {this.getEditPageDisplayedRelations().map(
                                        this
                                            .renderDraggableAttrEditSettingsRelation
                                    )}
                                    {/* DRAGGABLE BLOCK */}
                                    <div
                                        className={cn(
                                        ...
                                        >
                                            ...
                                            <DropdownMenu>
                                                {this.getDropDownRelationsItems().map(
                                                    this
                                                        .renderDropDownItemEditSettingsRelation
                                                )}
                                            </DropdownMenu>
                                        </ButtonDropdown>
                                    </div>
                    ...
                    <div className="col-md-8">
                        ...
                        >
                            <div className="row">
                                ...
                                {this.isEditingTextField() &&
                                    this.renderInputWysiwyg()}
                            </div>
                        </div>
                    </div>
                </div>
            ...
            >
                ...
                <div className="row">
                    ...
                    <div className="col-md-5">
                        {this.getListDisplay().map(
                            this.renderDraggableAttrListSettings
                        )}
                        <div
                            className={cn(
                                styles.dropdownWrapper,
                    ...
                    <div className="col-md-7">
                        <div className={styles.editWrapper}>
                            <div className="row">
                                {forms.editList.map(
                                    this.renderFormListAttrSettings
                                )}
                            </div>
                        </div>
                    </div>
    ...
    renderInputWysiwyg = () => {
        ...
        const name = `layout.${endPoint ||
            slug}.attributes.${inputName}.appearance`;
        const value =
            get(this.getLayout(), [inputName, 'appearance']) === 'WYSIWYG';

        return (
            <Input
                label={{ id: 'content-manager.form.Input.wysiwyg' }}
                type="toggle"
                name={name}
            ...
            <form onSubmit={this.handleSubmit}>
                ...
                <div className={cn('container-fluid', styles.containerFluid)}>
                    <PluginHeader
                        ...
                        title={`Content Manager - ${upperFirst(
                            this.getModelName()
                        )}`}
                        description={{
                            id:
