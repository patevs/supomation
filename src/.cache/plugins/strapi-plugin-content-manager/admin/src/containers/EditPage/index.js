...
export class EditPage extends React.Component {
    ...
    componentDidUpdate(prevProps) {
        ...
        if (
            prevProps.editPage.submitSuccess !==
            this.props.editPage.submitSuccess
        ) {
            if (
    ...
    initComponent = props => {
        ...
        if (!this.isCreating()) {
            const mainField =
                get(this.getModel(), 'info.mainField') ||
                this.getModel().primaryKey;
            this.props.getData(
    ...
    handleBlur = ({ target }) => {
        const defaultValue = get(
            this.getModelAttribute(target.name),
            'default'
        );
    ...
    isRelationComponentNull = () =>
        Object.keys(get(this.getSchema(), 'relations', {})).filter(
            relation =>
                get(this.getSchema(), ['relations', relation, 'plugin']) !==
                    'upload' &&
    ...
    retrieveLinksContainerComponent = () => {
        ...
        const componentToInject = Object.keys(appPlugins).reduce(
            (acc, current) => {
                    ...
                    .map(compo => {
                        ...
                        return (
                            <Component
                                ...
                                onClick={() => {
                                    emitEvent(
                                        'willEditContentTypeFromEditView'
                                    );
                                }}
                            />
                        );
                                    ...
                                    {this.shouldDisplayedRelations() && (
                                        <div className={styles.sub_wrapper}>
                                            <EditRelations
                                                ...
                                                isDraggingSibling={
                                                    editPage.isDraggingSibling
                                                }
                                                location={this.props.location}
                                                moveAttr={moveAttr}
                                                moveAttrEnd={moveAttrEnd}
                                                onAddRelationalItem={
                                                    this.handleAddRelationItem
                                                }
