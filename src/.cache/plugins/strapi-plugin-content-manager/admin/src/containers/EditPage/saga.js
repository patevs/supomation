...
export function* submit(action) {
    ...
    try {
        ...
        const recordCleaned = Object.keys(record).reduce((acc, current) => {
            ...
            switch (attrType) {
                ...
                case 'date':
                    cleanedData =
                        record[current] &&
                        record[current]._isAMomentObject === true
        ...
        if (isArray(get(err, 'response.payload.message'))) {
            const errors = err.response.payload.message.reduce(
                (acc, current) => {
                    const error = current.messages.reduce(
                        ...
                        {
                            id: 'components.Input.error.custom-error',
                            errorMessage: ''
                        }
                    );
                    acc.push(error);
        ...
        const notifErrorPrefix =
            ...
            ('admin' && shouldAddTranslationSuffix)
                ? 'users-permissions.'
                : '';
