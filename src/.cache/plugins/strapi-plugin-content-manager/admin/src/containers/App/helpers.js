...
const getElementsOnALine = (manager, line, list) => {
    const firstElIndex =
        line === 0
            ? 0
            : get(
                  manager.arrayOfEndLineElements[line - 1],
                  'index',
                  list.size - 1
              ) + 1;
    const lastElIndex =
        get(manager.arrayOfEndLineElements[line], 'index', list.size - 1) + 1;
    const elements = manager.getElementsOnALine(
        range(firstElIndex, lastElIndex)
    );
...
const reorderList = (manager, list) => {
    ...
    const uniqueIdList = reordered.reduce((acc, current, index) => {
        if (reordered.indexOf(current) === index) {
            ...
            const bootstrapCol = parseInt(
                current.split('__')[1].split('-')[2],
                10
            );
