...
function appReducer(state = initialState, action) {
  switch (action.type) {
    ...
    case END_MOVE:
      return state
        .updateIn(
          ['modifiedSchema', 'models', ...action.keys.split('.'), 'fields'],
          list => {
            ...
            if (shouldUpdateListOnDrop && canDrop) {
              ...
              const dropLine = findIndex(arrayOfLastLineElements, [
                'index',
                nodeBound.index
              ]);
              ...
              if (dropLine === initDragLine) {
                const toDropIndex =
                  dropIndex > addedElementIndex ? dropIndex + 1 : dropIndex;
              ...
              const { elements: previousStateLineEls } = getElementsOnALine(
                createManager(state, list, action.keys, dropIndex, layout),
                dropLine,
                list
              );
              const { elements: currentStateLineEls } = getElementsOnALine(
                newManager,
                dropLine,
                newList
              );
    ...
    case MOVE_ATTR:
      return state.updateIn(
        ['modifiedSchema', 'models', ...action.keys.split('.'), 'listDisplay'],
        list =>
          list
          ...
          list => {
            ...
            if (isFullSize && draggedItemIndex !== -1) {
              ...
              const indexToDrop = upwards
                ...
                : get(dropLineBounds, 'right.index', list.size - 1);
            ...
            if (!hasMoved && !isFullSize && draggedItemIndex !== -1) {
              const nodeBound = manager.getBound(true);
              const currentLine = findIndex(arrayOfLastLineElements, [
                'index',
                nodeBound.index
              ]);
            ...
            return acc
              ...
              .reduce((acc1, curr) => {
                return acc1
                  ...
                  .reduce((acc2, curr1) => {
                    return acc2.setIn(
                      [current, curr, curr1, action.keys[1]],
                      action.value
                    );
                  }, acc1);
              }, acc);
      ...
      return state.updateIn(
        ...
        list => {
          ...
          if (list.size - 1 === 0) {
            ...
            return list.delete(action.index).push(attrToAdd.get('0'));
          }
        ...
        .updateIn(
          ...
          list => {
            ...
            if (isRemovingAFullWidthNode) {
              ...
              const currentNodeLine = findIndex(arrayOfLastLineElements, [
                'index',
              ...
              if (currentNodeLine === 0) {
                ...
                const firstElementOnLine =
                  previousNodeLine === 0
                    ? 0
                    : arrayOfLastLineElements[previousNodeLine - 1].index + 1;
                const lastElementOnLine =
                  arrayOfLastLineElements[previousNodeLine].index + 1;
                const previousLineRangeIndexes =
                  firstElementOnLine === lastElementOnLine
                    ? [firstElementOnLine]
                    : range(firstElementOnLine, lastElementOnLine);
              ...
              const isRemovingLine =
                currentLineColSize - attrToRemoveInfos.bootstrapCol === 0;
