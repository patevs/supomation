...
function appReducer(state = initialState, action) {
    switch (action.type) {
        ...
        case END_MOVE:
            return state
                .updateIn(
                    ...
                    list => {
                        const shouldUpdateListOnDrop = state.get(
                            'shouldUpdateListOnDrop'
                        );
                        ...
                        if (shouldUpdateListOnDrop && canDrop) {
                            newList = list.insert(dropIndex, toAdd);

                            const addedElementName = state.get(
                                'addedElementName'
                            );
                            ...
                            const arrayOfLastLineElements =
                                manager.arrayOfEndLineElements;
                            const nodeBound = manager.getBound(true);
                            const dropLine = findIndex(
                                arrayOfLastLineElements,
                                ['index', nodeBound.index]
                            );
                            const addedElementIndex = newList.indexOf(
                                addedElementName
                            );
                            ...
                            if (dropLine !== initDragLine) {
                                const diff = difference(
                                    previousStateLineEls,
                                    currentStateLineEls
                                );
                                const diffLineSize = newManager.getLineSize(
                                    diff
                                );
                                const lineToCreate = [
                                    ...diff,
                                    ...manager.getColsToAdd(12 - diffLineSize)
                                ];
                            ...
                            const lastManager = createManager(
                                state,
                                newList,
                                action.keys,
                                dropIndex,
                            ...
                            newList = createManager(
                                state,
                                reorderList(lastManager, newList),
                                action.keys,
                                dropIndex,
                                layout
                            ).getLayout();
        ...
        case MOVE_ATTR_EDIT_VIEW:
            return state.updateIn(
                ['modifiedSchema', 'models', ...action.keys.split('.')],
                list =>
                    list
                        .delete(action.dragIndex)
                        .insert(action.hoverIndex, list.get(action.dragIndex))
            ...
            return state
                .updateIn(
                    ...
                    list => {
                        ...
                        const arrayOfLastLineElements =
                            manager.arrayOfEndLineElements;
                        const itemInfos = manager.getAttrInfos(
                            draggedItemIndex
                        );
                        ...
                        if (isFullSize && draggedItemIndex !== -1) {
                            ...
                            const indexToDrop = upwards
                                ? get(dropLineBounds, 'left.index', 0)
                                : get(
                                      dropLineBounds,
                                      'right.index',
                                      list.size - 1
        ...
        case ON_CHANGE:
            return state
                .updateIn(
                    ['modifiedSchema'].concat(action.keys),
                    () => action.value
                )
                .updateIn(['modifiedSchema', 'models'], models => {
                    return models.keySeq().reduce((acc, current) => {
                        if (current !== 'plugins') {
                            return acc.setIn(
                                [current, action.keys[1]],
                                action.value
                            );
                        }
        ...
        case ON_CHANGE_INPUT_TYPE:
            return state
                .updateIn(
                    ['modifiedSchema', ...action.keys],
                    () => action.value
                )
                .update('shouldResetGrid', v => !v);
        case ON_CHANGE_SETTINGS:
            return state.updateIn(
                ['modifiedSchema', 'models'].concat(action.keys),
                () => action.value
            );
        case ON_CLICK_ADD_ATTR:
            return state.updateIn(
                ['modifiedSchema', 'models', ...action.keys.split('.')],
                list => list.push(fromJS(action.data))
            );
            ...
            return state
                .updateIn(
                    ...
                    list => {
                        ...
                        const arrayOfLastLineElements =
                            manager.arrayOfEndLineElements;
                        const isRemovingAFullWidthNode =
                            attrToRemoveInfos.bootstrapCol === 12;
                        let newList;

                        if (isRemovingAFullWidthNode) {
                            // If removing we need to add the corresponding missing col in the prev line
                            const currentNodeLine = findIndex(
                                arrayOfLastLineElements,
                                ['index', attrToRemoveInfos.index]
                            ); // Used only to know if removing a full size element on the first line

                            if (currentNodeLine === 0) {
                                newList = list.delete(action.index);
                            } else {
                                ...
                                const lastElementOnLine =
                                    arrayOfLastLineElements[previousNodeLine]
                                        .index + 1;
                                ...
                                const elementsOnLine = manager.getElementsOnALine(
                                    previousLineRangeIndexes
                                );
                                const previousLineColNumber = manager.getLineSize(
                                    elementsOnLine
                                );
                                ...
                                if (previousLineColNumber >= 10) {
                                    ...
                                    const colNumberToAdd =
                                        12 - previousLineColNumber;
                                    const colsToAdd = manager.getColsToAdd(
                                        colNumberToAdd
                                    );
                                    newList = list
                                        .delete(attrToRemoveInfos.index)
                                        .insert(
                                            attrToRemoveInfos.index,
                                            colsToAdd[0]
                                        );
                            ...
                            const nodeBounds = {
                                left: manager.getBound(false),
                                right: manager.getBound(true)
                            }; // Retrieve the removed element's bounds
                            const leftBoundIndex =
                                get(nodeBounds, ['left', 'index'], 0) + 1;
                            const rightBoundIndex = get(
                                nodeBounds,
                                ['right', 'index'],
                                list.size - 1
                            );
                            const elementsOnLine = manager.getElementsOnALine(
                                range(leftBoundIndex - 1, rightBoundIndex + 1)
                            );
                            const currentLineColSize = manager.getLineSize(
                                elementsOnLine
                            );
                            const isRemovingLine =
                                currentLineColSize -
                                    attrToRemoveInfos.bootstrapCol ===
                                0;

                            if (isRemovingLine) {
                                newList = list.delete(attrToRemoveInfos.index);
                            } else {
                                const random = Math.floor(Math.random() * 1000);
                                newList = list
                                    .delete(attrToRemoveInfos.index)
                ...
                .updateIn(
                    ...
                    relation => {
                        return relation
                            .update('description', () => '')
                            .update('label', () =>
                                upperFirst(relation.get('alias'))
                            );
                    }
                )
                .updateIn(
                    ['modifiedSchema', 'models'].concat(action.keys.split('.')),
                    list => {
                        return list.delete(action.index);
                    }
                );
            ...
            return state
                ...
                .update('grid', () => {
                    ...
                    const lines = getLines(
                        new Manager(state, fields, action.keys, 0, layout),
                        fields
                    );
