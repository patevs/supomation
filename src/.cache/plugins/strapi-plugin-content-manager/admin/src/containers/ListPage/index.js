...
export class ListPage extends React.Component {
    ...
    componentDidUpdate(prevProps) {
        ...
        if (
            prevProps.listPage.didChangeDisplayedFields !==
            didChangeDisplayedFields
        ) {
            const dataToStore = {
    ...
    handleChangeHeader = ({ target }) => {
        const defaultSettingsDisplay = get(this.getCurrentModel(), [
            'listDisplay'
        ]);
        ...
        if (attrIndex !== -1) {
            if (get(this.props.listPage, 'displayedFields', []).length === 1) {
                ...
                const isRemovingDefaultSort =
                    get(this.props.listPage, ['params', '_sort']) ===
                    target.name;
                let newDefaultSort;
                ...
                if (isRemovingDefaultSort) {
                    ...
                    this.handleChangeSort(
                        newDefaultSort || this.getModelPrimaryKey()
                    );
                }
    ...
    showLoaders = () => {
        ...
        return (
            updatingParams ||
            (isLoading &&
                get(records, this.getCurrentModelName()) === undefined)
        );
    };
    ...
    showFilters = () => {
        if (
            isEmpty(
                get(this.getCurrentModel(), ['editDisplay', 'availableFields'])
            )
        ) {
            return false;
        }
    ...
    renderDropdown = item => {
        return (
            <DropdownItem
                ...
                toggle={false}
                onClick={() =>
                    this.handleChangeHeader({ target: { name: item } })
                }
            >
                <div>
                    <InputCheckbox
        ...
        const pluginHeaderActions = [
            {
                ...
                labelValues: {
                    entity:
                        capitalize(this.props.match.params.slug) ||
                        'Content Manager'
                },
