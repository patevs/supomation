...
class SelectMany extends React.PureComponent {
    ...
    getOptions = query => {
        ...
        if (query) {
            ...
            delete params._skip;
            params[
                `${this.props.relation.displayedAttribute}_contains`
            ] = query;
        }
        // Request URL
        const requestUrl = `/content-manager/explorer/${this.props.relation
            .model || this.props.relation.collection}`;
                    ...
                    : [
                          {
                              ...
                              label:
                                  response[
                                      this.props.relation.displayedAttribute
                                  ]
                          }
                      ];
    ...
    handleChange = value => {
        ...
        this.state.options = this.state.options.filter(
            el =>
                !(
                    (el.value._id || el.value.id) ===
                    (value.value.id || value.value._id)
                )
        );
    ...
    render() {
        ...
        return (
            <div
                className={`form-group ${styles.selectMany} ${value.length >
                    4 && styles.selectManyUpdate}`}
            >
