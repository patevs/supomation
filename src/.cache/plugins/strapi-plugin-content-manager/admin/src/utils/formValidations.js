        ...
        switch (validationKey) {
            ...
            case 'maxLength':
                if (value && value.length > validationValue) {
                    errors.push({
                        id: 'content-manager.error.validation.maxLength'
                    });
                }
                break;
            case 'minLength':
                if (value && value.length < validationValue) {
                    errors.push({
                        id: 'content-manager.error.validation.minLength'
                    });
                }
                break;
            case 'required':
                if (validationValue === true && value.length === 0) {
                    errors.push({
                        id: 'content-manager.error.validation.required'
                    });
                }
                break;
            case 'regex':
                if (!new RegExp(validationValue).test(value)) {
                    errors.push({
                        id: 'content-manager.error.validation.regex'
                    });
                }
                break;
            case 'type':
                if (validationValue === 'json') {
                    try {
                        if (
                            ...
                            errors.push({
                                id: 'content-manager.error.validation.json'
                            });
                        }
                    } catch (err) {
                        errors.push({
