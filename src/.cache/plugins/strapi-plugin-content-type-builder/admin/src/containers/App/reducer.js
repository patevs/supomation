    ...
    switch (action.type) {
            ...
            return state
                .removeIn([
                    ...
                    state
                        .get('models')
                        .findIndex(
                            model => model.get('name') === action.modelName
                        )
                ])
        ...
        case GET_DATA_SUCCEEDED:
            return state
                ...
                .updateIn(
                    ['newContentType', 'connection'],
                    () => action.connections[0]
                )
                .update('models', () =>
                    List(fromJS(action.models)).sortBy(model =>
                        model.get('name')
                    )
                );
        ...
        case ON_CHANGE_RELATION_NATURE: {
            ...
            return state
                .updateIn(
                    ['temporaryAttributeRelation', 'nature'],
                    () => nature
                )
        ...
        case RESET_EDIT_TEMP_CONTENT_TYPE:
            return state.updateIn(['newContentType', 'attributes'], () =>
                Map({})
            );
        case RESET_EXISTING_CONTENT_TYPE_MAIN_INFOS:
            return state.updateIn(
                ['modifiedData', action.contentTypeName],
                () => {
        ...
        case RESET_NEW_CONTENT_TYPE_MAIN_INFOS:
            return state.updateIn(['newContentType'], () => {
                const initialContentType = state
                    ...
                    .set(
                        'attributes',
                        state.getIn(['newContentType', 'attributes'])
                    );
        ...
        case SAVE_EDITED_ATTRIBUTE_RELATION: {
            ...
            const updatedRelationNature = temporaryAttributeRelation.get(
                'nature'
            );
            const updatedRelationTarget = temporaryAttributeRelation.get(
                'target'
            );
            ...
            return state
                .updateIn(
                    [
                        ...
                        state
                            .get('models')
                            .findIndex(model => model.isTemporary === true),
                        'name'
                    ],
                    () => state.getIn(['newContentType', 'name'])
                )
