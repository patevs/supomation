...
class RelationForm extends React.Component {
    ...
    getFormErrors = () => {
        ...
        const alreadyTakenAttributesUpdated = alreadyTakenAttributes.filter(
            attribute => {
                if (actionType === 'edit') {
                    return (
                        attribute !== attributeToEditName &&
                        attribute !== modifiedData.key
                    );
                }
        ...
        if (modifiedData.name === modifiedData.key) {
            formErrors = {
                key: [{ id: `${pluginId}.error.attribute.key.taken` }]
            };
        }
        ...
        formErrors = Object.keys(formValidations).reduce((acc, current) => {
            ...
            if (required === true && !value) {
                acc[current] = [
                    { id: `${pluginId}.error.validation.required` }
                ];
            }

            if (
                unique === true &&
                alreadyTakenAttributesUpdated.includes(value)
            ) {
                acc[current] = [
                    { id: `${pluginId}.error.attribute.key.taken` }
                ];
            }
    ...
    handleGoTo = to => {
        ...
        const attributeName =
            actionType === 'edit'
                ? `&attributeName=${attributeToEditName}`
                : '';
