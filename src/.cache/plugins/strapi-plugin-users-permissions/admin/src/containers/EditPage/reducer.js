...
function editPageReducer(state = initialState, action) {
    switch (action.type) {
        case ADD_USER:
            return state.updateIn(['modifiedData', 'users'], list =>
                list.push(action.newUser)
            );
            ...
            return state
                ...
                .updateIn(['modifiedData', 'users'], list =>
                    list.filter(
                        o =>
                            o[o.id ? 'id' : '_id'] !==
                            action.itemToDelete[o.id ? 'id' : '_id']
                    )
                );
        case RESET_PROPS:
            return state
        ...
        case SELECT_ALL_ACTIONS: {
            ...
            return state.updateIn(action.keys, () =>
                Map(fromJS(controllerActions))
            );
        }
