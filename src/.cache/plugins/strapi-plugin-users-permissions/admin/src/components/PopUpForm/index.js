...
class PopUpForm extends React.Component {
    ...
    renderForm = () => {
        ...
        const form = Object.keys(values.options || values || {}).reduce(
            (acc, current) => {
                const path =
                    settingType === 'email-templates'
                        ? ['options', current]
                        : [current];
                const name =
                    settingType === 'email-templates' ? 'options.' : '';

                if (
                    isObject(get(values, path)) &&
                    !isArray(get(values, path))
                ) {
                    ...
                    {map(tail(form), (value, key) => (
                        <Input
                            ...
                            errors={get(
                                formErrors,
                                [
                                    findIndex(formErrors, ['name', value]),
                                    'errors'
                                ],
                                []
                            )}
                ...
                {map(take(form, 3), (value, key) => (
                    <Input
                        ...
                        errors={get(
                            ...
                            [
                                findIndex(this.props.formErrors, [
                                    'name',
                                    value
                                ]),
                                'errors'
