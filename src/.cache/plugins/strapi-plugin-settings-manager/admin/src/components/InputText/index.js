...
class InputText extends React.Component {
    ...
    handleBlur = ({ target }) => {
        ...
        if (!isEmpty(target.value) || this.state.hasInitialValue) {
            ...
            const errors =
                indexErrorDbExist !== -1
                    ? uniqBy(
                          union(this.props.errors, this.validate(target.value)),
                          'id'
                      )
            ...
            switch (validationKey) {
                case 'maxLength':
                    if (value.length > validationValue) {
                        errors.push({
                            id:
                                'settings-manager.request.error.validation.maxLength'
                        });
                    }
                    break;
                case 'minLength':
                    if (value.length < validationValue) {
                        errors.push({
                            id:
                                'settings-manager.request.error.validation.minLength'
                        });
                    }
                    break;
                case 'required':
                    if (value.length === 0) {
                        errors.push({
                            id:
                                'settings-manager.request.error.validation.required'
                        });
                    }
                    break;
                case 'regex':
                    if (!new RegExp(validationValue).test(value)) {
                        errors.push({
                            id:
                                'settings-manager.request.error.validation.regex'
                        });
                    }
                    break;
                default:
    ...
    render() {
        ...
        const spacer = !this.props.name
            ? { marginTop: '2.4rem' }
            : { marginTop: '' };
        const marginBottomInput = isEmpty(this.state.errors)
            ? '4.3rem'
            : '2.4rem';
