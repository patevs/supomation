...
function scrapPage(prompt, targetPageUrl) {
	...
	axios
		...
		.then(function (response) {
			// Handle success
			prompt.success("[%d/2] - Navigated to target page: %s\n", 2, logging.link(TARGET_URL));
			// TODO: return page html content
			pageHtmlContent = response.data; // Target page HTML content
		})
		.catch(function (error) {
			// Handle error
			prompt.error("[%d/2] - Error navigating to target page! %s", 2, logging.logError(error));
...
function scrapCategory(categoryUrl) {
	// ..
	const navPrompt = new Signale({ interactive: true, scope: "supomation" });
	navPrompt.await("[%d/2] - Navigating to category: %s", 1, logging.link(categoryUrl));
	scrapPage(navPrompt, categoryUrl);
	// ..
}
