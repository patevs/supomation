	...
	axios
		...
		.finally(function () {
			...
			if (pageHtmlContent === null) {
				...
				prompt.success("[%d/2] - Scraped %s products from target page!", 2, logging.green(numProducts));
				saveProductData("test.json", scrapedProducts);
			}
...
async function saveProductData(fileName, jsonData) {
	...
	logging.logInfo("SAVE DATA TO: " + filePath);
}
