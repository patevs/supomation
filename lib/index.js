...
async function runWebscraper() {
	...
	axios
		...
		.finally(function () {
			...
			savePrompt.await("[%d/2] - Saving products data to file: %s", 1, filePath);
			saveProductData("data/test.json", { foo: true });
			savePrompt.success("[%d/2] - Saved products data to file: %s", 2, filePath);
