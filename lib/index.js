...
function runWebscraper() {
	...
	scrapPrompt.await("[%d/2] - Navigating to target page: " + link(TARGET_URL), 1);
	...
	axios.get(TARGET_URL)
		.then(function (response) {
			...
			scrapPrompt.success("[%d/2] - Navigated to target page: " + link(TARGET_URL), 2);
		...
		.finally(function () {
			// Always executed
			// . scrapPrompt.await("[%d/2] - Scraping products from target page...", 2);
